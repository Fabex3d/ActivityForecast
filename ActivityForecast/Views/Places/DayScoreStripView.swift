    ...
    var body: some View {
        Group {
            if dynamicTypeSize.isAccessibilitySize {
                // Seven columns cannot share a phone's width at accessibility sizes.
                // Scrolling keeps every day readable instead of shrinking the text.
...
#Preview("Skiing — strong start, thin middle") {
    DayScoreStripView(days: PreviewData.zermattWeek, activity: .skiing)
        .padding(Spacing.standard)
        .screenBackground()
}

#Preview("Outdoor sights — a poor week") {
    DayScoreStripView(days: PreviewData.zermattWeek, activity: .outdoorSightseeing)
        .padding(Spacing.standard)
        .screenBackground()
}

#Preview("Accessibility text size") {
    DayScoreStripView(days: PreviewData.losAndesWeek, activity: .outdoorSightseeing)
        .padding(Spacing.standard)
        .screenBackground()
