        ...
        var body: some View {
            HStack(alignment: .top, spacing: Spacing.medium) {
                VStack(alignment: .leading, spacing: Spacing.small) {
                    ...
                    Label(
                        leadingDay?.conditionsSummary ?? place.subtitle,
                        systemImage: leadingDay?.condition.systemImage ?? ""
                    )
...
#Preview("Loading and failed") {
    VStack(spacing: Spacing.medium) {
        PlacesView.CardView(place: .previewKyoto, state: .loading, retry: {})
