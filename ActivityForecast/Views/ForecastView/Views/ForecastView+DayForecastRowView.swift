...
extension ForecastView.DayForecastRowView {
...
#Preview("A week of skiing") {
    List(PreviewData.zermattWeek) { day in
        ForecastView.DayForecastRowView(day: day, activity: .skiing)
    }
    .listStyle(.plain)
}
...
#Preview("A week of outdoor sights — mostly poor") {
    List(PreviewData.zermattWeek) { day in
        ForecastView.DayForecastRowView(day: day, activity: .outdoorSightseeing)
    }
    .listStyle(.plain)
}
